...
export class AdicionarReceitaPage implements OnInit {
  ...
  loadProduct() {
    this.productSubscription = this.productService.getReceita(this.productId).subscribe(data => {
      this.product = data;
    });
  }
  ...
  async saveProduct() {
    ...
    if (this.productId) {
      try {
        await this.productService.updateReceita(this.productId, this.product);
      ...
      try {
        await this.productService.addReceita(this.product);
        await this.loading.dismiss();
